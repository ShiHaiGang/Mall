...
export default {
  ...
  LocalStorage(method, key, value, obj) {
    if (typeof Storage !== "undefined") {
      // 存储数据的两个对象
      var local = obj != "session" ? localStorage : sessionStorage;
      // 常用API
  ...
  MoneyTransform(money) {
    try {
      /* eslint-disable */
      var MaxUnits = new Array("", "万", "亿", "兆", "京", "垓", "杼", "穰", "沟", "涧", "正"), //大单位
      ...
      var IntegerString, //存储整数部分
        DecimalString; //存储小数部分

      var OutputNegative = "", //输出金额负数部分
        OutputInteger = "", //输出金额整数部分
        OutputDecimal = ""; //输出金额小数部分
      ...
      /*转换整数部分*/
      for (var i = 1; i <= IntegerString.length; i++) {
      ...
      if (DecimalString != undefined) {
        for (var j = 0; j <= DecimalString.length; j++) {
